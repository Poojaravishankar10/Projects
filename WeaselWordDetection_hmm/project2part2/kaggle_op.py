...
def gen_kaggle_op_lists(op_path, amb_sent_threshold):
   ...
   for file in all_text_files:
       ...
       for line in read_handle:
           if not line.strip():
               ...
               # chosen.
               #
               if(amb_words_per_sent_count > amb_sent_threshold):
           ...
           if('I' == line_split[2] or 'B' == line_split[2]):
...
def gen_kaggle_file(public_path, private_path, file_write_path, extra_tag, only_public = 0, amb_sent_threshold =1):
   word_file_content = "Type,Spans\n";
   sent_file_content = "Type,Indices\n";

   public_output = gen_kaggle_op_lists(public_path, amb_sent_threshold);
   word_file_content += "CUE-public,"+"".join(public_output[0][1:])+"\n";
   sent_file_content += "SENTENCE-public," + "".join(public_output[1])+"\n";

   if(not only_public):
       private_output = gen_kaggle_op_lists(private_path, amb_sent_threshold);
